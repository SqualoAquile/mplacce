...
class Fornecedores extends model {
    ...
    public function nomeFornecedores($request){
        $termo = trim(addslashes($request["term"]));
        $negocio = trim(addslashes($request["negocio"]));
        // print_r($termo); exit;
        // echo "aquiiii"; exit;
        $array = array();
        // 
        $sql1 = "SELECT * FROM `fornecedores` WHERE situacao = 'ativo' AND negocio = '$negocio' AND forn_ativo = 'SIM' AND ( nome_fantasia LIKE '%$termo%' OR razao_social LIKE '%$termo%' ) ORDER BY nome_fantasia ASC";
